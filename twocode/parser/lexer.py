...
class LexLanguage:
    ...
    def form_model(self):
        lex_model = LexModel()
        for pattern in self.preprocessor:
            lex_model.add_rule(pattern, "none")
        ...
        EOL = r'\n|\r\n|\r'
        lex_model.add_rule(r'\\({})'.format(EOL), "none")
        # REASON: ignore \<EOL>
        lex_model.add_rule(r'({})[ \t]*(?={}|$)'.format(EOL, EOL), "line")
        # REASON: empty line to EOL
        if self.indent_block:
            lex_model.add_rule(r'({})[ \t]*.'.format(EOL), "indent")
        lex_model.add_rule(EOL, "line")
        lex_model.add_rule(r'[ \t]+', "token" if self.allow_ws else "none", "WS")
        lex_model.add_rule(r'.|\n', "error") # it already throws an error though?
        return lex_model
